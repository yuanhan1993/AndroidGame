 ...
 *
 */
public class GameView5 extends View{
	...
	public GameView5(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public void draw(Canvas canvas) {
		...
		// 绘制下一轮要显示的三个珠子
		List<Bead> lists =  gameService5.getPreparedBeads();
		...
		for (int i = 0; i < beadBoard.beads.length; i++){
			for (int j = 0; j < beadBoard.beads.length; j++){
				...
				if (bead.getBitmap() != null){ // 绘制珠子
					// 获取可消珠子连接点
					List<Point> points = gameService5.getLinkPoints();
	...
	public void setGameService(GameService5 gameService5) {
		this.gameService5 = gameService5;
	}
