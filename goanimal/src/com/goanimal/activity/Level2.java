...
import com.goanimal.service.impl.GameServiceImpl2;
...
import com.goanimal.view.GameView2;
...
public class Level2 extends Activity{
	...
	// 定义游戏的业务对象
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			// 获取游戏的主界面
			Level2.this.gameView2 = (GameView2)findViewById(R.id.gameView2);
			  
           
			// 创建业务对象
			gameService2 = (GameService2) new GameServiceImpl2(this, gameView2.getBeadBoard());
			// 设置业务对象
			gameView2.setGameService(gameService2);
			Log.i("here", "就这了");
			// 为gameView绑定触摸事件
			gameView2.setOnTouchListener(new View.OnTouchListener() {
				...
				public boolean onTouch(View view, MotionEvent event) {
					...
					if (event.getAction() == MotionEvent.ACTION_DOWN){
						...
						// 根据用户点击的坐标获取相对应的珠子
						Bead bead = gameService2.getSelectedBead(event.getX(), event.getY());
						// 用户选中的珠子(要走的珠子)
						if (bead != null && bead.getBitmap() != null && lock){
							// 为gameView设置选中的珠子
							gameView2.setSelectedBead(bead);
							// 要让选中的珠子跳动
							...
							// 珠子存在,但是没有位图
							if (bead != null && gameView2.getSelectedBead() != null){
								// 获取两个珠子之间的线路
								points = gameService2.getPath(gameView2.getSelectedBead(), bead);
								if (!points.isEmpty()){
									// 为gameView设置目的珠子
									gameView2.setTargetBead(bead);
									// 加锁
									lock = false;
									// 开启动画效果走珠子
				...
				switch (msg.what){
					...
					case Constant.FLAG_4: // 代表消珠子
						if (msg.obj != null){
							...
							// 获取分数
							int score = gameService2.getPerScore();
							if (score > 0){
								// 提示得分
								Toast.makeText(Level2.this, "+" + score, Toast.LENGTH_SHORT).show();
								// 设置分数累加
								gameService2.setTotalScore();
								int totalScore = gameView2.getBeadBoard().getTotalScore();
								if(totalScore>=50){
									Toast.makeText(Level2.this, "恭喜开启下一关", Toast.LENGTH_SHORT).show();
		...
		private void autoScan(int scanType) {
			// 消珠子(有珠子可消，就不要显示三个珠子)
			if (gameService2.scanBead(scanType)){
				// 开启消珠子动画效果
				...
				if (scanType == 1){
					// 显示珠子
					displayBeads = gameService2.getDisplayBeads();
					...
					// 判断珠盘上是否有空珠子
					if (gameService2.getEmptyBeads().size() == 0){
						// 游戏结束
						gameOver();
					}
					// 解锁
		...
		private void gameOver() {
			...
			int totalScore = gameView2.getBeadBoard().getTotalScore();
			// 获取历史成绩
			int histScore = gameView2.getBeadBoard().getHistScore();
			// 提示本次游戏总得分
			...
			if (totalScore > histScore){
				...
				// 设置历史分数
				gameView2.getBeadBoard().setHistScore(totalScore);
			}
			// 游戏重新开始
			gameService2.reset();
			gameView2.postInvalidate();
		...
		public boolean onOptionsItemSelected(MenuItem item) {
			switch (item.getItemId()){
				case R.id.returnMenu: // 返回菜单
					Intent intent1=new Intent(Level2.this,MainMenu.class);
					startActivity(intent1);
					break;
				case R.id.nextLevel: // 选关
					Intent intent2=new Intent(Level2.this,selectLevel.class);
